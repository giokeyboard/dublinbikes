...
# prepare a cursor object using cursor() method
    ...
    for i in range(0,len(value)):
        ...
        sql = """INSERT INTO dublinbikes.availability (number,available_bikes,available_bike_stands,last_update,day_of_week) VALUES ('%s','%s','%s','%s','%s')""" % (number_d,available_bikes_d,available_bike_stands_d,last_update_d,day)
        try:
            print("Line 1 ")
            cursor.execute(sql) # Execute the SQL command
            print(cursor._last_executed)
